...
from .parser.expressoVisitor import expressoVisitor
from typing import List
import ast

class astVisitor(expressoVisitor):
    ...
    def visitFunc(self, ctx:expressoParser.FuncContext):
        name = ctx.name.text
    ...
    def visitStatms(self, ctx:expressoParser.StatmsContext):
        return [self.visit(stmt) for stmt in ctx.statm()]

    def visitWhile(self, ctx:expressoParser.WhileContext):
        test = self.visit(ctx.cond)
        body = self.visit(ctx.statms())
        return ast.While(test=test, body=body, orelse=[])

    def visitIf(self, ctx:expressoParser.IfContext):
        test = self.visit(ctx.cond)
    ...
    def visitCall(self, ctx:expressoParser.CallContext):
        id = ctx.name.text
    ...
    def visitExprs(self, ctx:expressoParser.ExprsContext):
        return [self.visit(x) for x in ctx.expr()]

    def visitAssign(self, ctx:expressoParser.AssignContext):
    ...
    def visitReturn(self, ctx:expressoParser.ReturnContext):
    ...
    def visitExpr(self, ctx:expressoParser.ExprContext):
    ...
    def visitSumm(self, ctx:expressoParser.SummContext):
        left = self.visit(ctx.left)
        if ctx.right:
            right = self.visit(ctx.right)
            if ctx.op.text == '+':
    ...
    def visitMult(self, ctx:expressoParser.MultContext):
        left = self.visit(ctx.left)
        if ctx.right:
            right = self.visit(ctx.right)
            if ctx.op.text == '*':
    ...
    def visitPrint(self, ctx:expressoParser.PrintContext):
